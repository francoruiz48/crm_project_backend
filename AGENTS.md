...
- Va resolviendo los hallazgos de la auditoría **de a uno, en orden de impacto**. Hallazgos #1 (nomencladores), #2 (estados de contacto) y #3 (storage/import sin autenticación/permiso) ya están RESUELTOS (código + test + docs). El siguiente en la lista, si pide continuar, es el #4 (`WebForm`/formularios públicos sin ningún test — no es un bug, es un hueco de cobertura).
