...
3. **CAPTCHA server-side** (opcional, si `form.require_captcha=True`): valida el token contra un servicio externo (Cloudflare Turnstile / reCAPTCHA v3 vía `settings.CAPTCHA_VERIFY_URL`, `timeout=10.0`). Sin token o con verificación fallida (el proveedor dice que no es válido), `400`. Si el proveedor externo falla (caído, timeout, respuesta no-JSON — hallazgo #10, ver §8), `503` — se distingue explícitamente de los `400`, que son responsabilidad del visitante.
...
- **[RESUELTO 2026-07-11] #10 — La verificación de CAPTCHA no manejaba errores.** La llamada al proveedor externo (`httpx`) no tenía `try/except` ni timeout explícito; si el proveedor fallaba o tardaba, el endpoint devolvía un `500` crudo en vez de un error prolijo. Fix: `try/except (httpx.HTTPError, ValueError)` + `timeout=10.0` explícito (ver §5) — ahora responde `503` controlado.
- **[RESUELTO 2026-07-11] #11 — `request.client.host` puede no ser la IP real si hay proxy delante.** El despliegue de producción todavía no está definido, pero se decidió asumir que va a haber un proxy delante (nginx, load balancer, etc. — es lo más probable) y centralizar la resolución de IP en un solo lugar. Fix: `get_client_ip` (`app/core/security.py`) prioriza `X-Forwarded-For` → `X-Real-IP` → `request.client.host`, usada tanto por el rate limit (`limiter = Limiter(key_func=get_client_ip)`) como por el `remoteip` que se le manda al proveedor de CAPTCHA. Mismo fix aplicado también al rate limit de `/auth/login`+`/auth/register` (hallazgo #12) y al limiter global — mismo bug raíz, ver `autenticacion.md` §11.
