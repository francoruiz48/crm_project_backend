...
**[RESUELTO 2026-07-11, hallazgo #23]** `POST /lead_flows/graph` ahora exige el permiso `lead_flow:update` (`PermissionChecker`), igual que el resto de las rutas de escritura de este módulo. Antes solo exigía estar autenticado — internamente ya era tenant-safe (todo se resuelve contra `organization_id` del propio usuario), pero cualquier rol, incluido `agent` (que no tiene `lead_flow:update`), podía rediseñar el flujo de ventas completo de su organización. Detalle en `hallazgos_agente/flujo_de_leads.md`.
...
- **[RESUELTO 2026-07-11, hallazgo #25]** `LeadFlowService.update` resolvía el objeto con query cruda sin filtro de tenant (mismo patrón sistémico que en `LeadContactState`/`Nomenclator`/`Tag` — ver `hallazgos_agente/patron_queries_sin_tenant_filter.md`). Un `obj_id` ajeno daba `500` en vez de `404`. Fix: ahora usa `cls.repository.get_by_id(...)`. Test de regresión: `tests/functional/test_tenant_isolation.py::TestLeadFlowIsolation::test_update_blocked_for_foreign_lead_flow`.
