...
- **[RESUELTO, hallazgo #12, 2026-07-10]** `/auth/login` y `/auth/register` tienen rate limiting (`10/minuto` por IP, vía `slowapi` — misma infraestructura que ya usaba `WebForm`). Antes no había ningún freno a la cantidad de intentos (fuerza bruta / credential stuffing en login, spam de cuentas en register). **[RESUELTO, hallazgo #11, 2026-07-11]** Ese `10/minuto` "por IP" usaba `request.client.host`, que sería la misma para todos los visitantes si hay un proxy/load balancer delante (rompiendo el límite: se vuelve compartido por todos en vez de individual). Se asumió que va a haber un proxy en producción (todavía no está definida la infraestructura) y se centralizó la resolución de IP en `get_client_ip` (`app/core/security.py`, prioriza `X-Forwarded-For` → `X-Real-IP` → `request.client.host`), usada ahora por el `Limiter` de este router. Mismo fix aplicado en `WebForm` y en el limiter global — ver `formularios_web.md` §8.
