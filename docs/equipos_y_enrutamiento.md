...
- **Permisos**: solo un `MANAGER` del `target_team_id` (o superadmin/owner) puede crear/actualizar/borrar/desactivar/reactivar una política que apunte a ese equipo (`_assert_manager`). **[RESUELTO, hallazgo #16, 2026-07-10]** Hasta esta fecha, `DELETE /{id}`, `PUT /active/{id}` y `DELETE /active/{id}` no aplicaban esta regla — cualquier miembro autenticado de la organización podía borrar/desactivar/reactivar cualquier política, sin importar su rol en el equipo destino. Detalle en `hallazgos_agente/equipos_y_enrutamiento.md`.
...
- **[RESUELTO, hallazgo #16, 2026-07-10]** `DELETE /lead_routing_policies/{id}`, `PUT /active/{id}` y `DELETE /active/{id}` ahora exigen ser `MANAGER` del equipo destino, igual que create/update. Ver `hallazgos_agente/equipos_y_enrutamiento.md`.
