...
class NomenclatorItemService(BaseService):
    ...
    def create(cls, obj_in, user_context: Optional[UserContext] = None):
        def do_create(uow):
            ...
            parent_item_objs = cls._resolve_and_validate_parent_items(
                uow.session, parent_nom, obj_in.parent_item_ids
            )
            ...
            if parent_item_objs:
                db_item = db_item or uow.session.query(NomenclatorItem).filter_by(id=new_item_response.id).first()
    ...
    def update(cls, obj_id: int, obj_in, user_context: Optional[UserContext] = None):
        def do_update(uow):
            ...
            current_item = cls.repository.get_by_id(uow.session, obj_id, user_context=user_context)
            if not current_item:
            ...
            if obj_in.parent_item_ids is not None:
                if obj_id in obj_in.parent_item_ids:
                    raise HTTPException(
                        status.HTTP_400_BAD_REQUEST,
                        detail=[{"field": "parent_item_ids", "message": "Un ítem no puede ser padre de sí mismo."}]
                    )
                if cls._would_create_cycle(uow.session, obj_id, obj_in.parent_item_ids):
                    raise HTTPException(
                        status.HTTP_400_BAD_REQUEST,
                ...
                parent_item_objs = cls._resolve_and_validate_parent_items(
                    uow.session, item_nomenclator, obj_in.parent_item_ids
                )

            # Actualización
            update_data = obj_in.model_dump(exclude={"parent_item_ids"}, exclude_unset=True)
            updated_item = cls.repository.update(uow.session, obj_id, update_data, user_context=user_context)
            uow.session.flush()

            if parent_item_objs is not None:
                db_item = uow.session.query(NomenclatorItem).filter_by(id=obj_id).first()
                db_item.parent_items = parent_item_objs
                uow.session.flush()
                uow.session.refresh(db_item)
